...
## Code from gstat
gstatkrg <- function(data, data.grid, formula, filename.out,filename.grid.out, var_model = "Sph"){
    ...
    data$res <- fit$res
    write.table(data, filename.out, sep = ",", col.names = TRUE, row.names = FALSE, quote = FALSE)

    # (Universal) Kriging
    krg <- krige(formula, data, data.grid, model = fit.vg)
    data.grid$mean <- krg$var1.pred
    data.grid$sd <- sqrt(krg$var1.var)
    write.table(data.grid, filename.grid.out, sep = ",", col.names = TRUE, row.names = FALSE, quote = FALSE)
}

get_INLAspde_results <-  function(data, data.grid, mesh, model, stack,response_var, filename.out, filename.grid.out, sp_res = c(300,300)){ 
    ...
    write.table(data_out,filename.out, sep = ",", col.names = TRUE, row.names = FALSE, quote = FALSE)
    write.table(data.grid_out,filename.grid.out, sep = ",", col.names = TRUE, row.names = FALSE, quote = FALSE)
    write.table(sp_out,gsub('.csv','_sp.csv',filename.out), sep = ",", col.names = TRUE, row.names = FALSE, quote = FALSE)
...
INLAspde <- function(data, data.grid, family, response_var, predictors, data.crs,filename.out, filename.grid.out){
    ...
    get_INLAspde_results(data, data.grid, mesh, model, stack.join,response_var, filename.out, filename.grid.out)
